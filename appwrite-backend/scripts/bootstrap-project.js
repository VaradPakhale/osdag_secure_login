...
// Self-hosted default. This script does NOT apply to Appwrite Cloud — Cloud has
// no API for creating a project, so that path uses the console. [ADR-0017]
...
async function main() {
  ...
  if (created.status === 201) {
    console.log('[bootstrap] console account created');
  } else if (created.status === 409) {
    console.log('[bootstrap] console account already exists');
  } else if (created.status === 501 && created.body?.type === 'user_console_count_exceeded') {
    // Appwrite closes console sign-up once a root account exists, and answers
    // 501 rather than 409 — even when the account we are asking for is the one
    // that already exists. Not an error for us: we only need to log in.
    console.log('[bootstrap] console sign-up is closed (root account exists) — logging in');
  } else {
    throw new Error(`account create failed: ${created.status} ${JSON.stringify(created.body)}`);
  }
  ...
  //
  // SESSION_ENCRYPTION_KEY is generated here too: config.js requires it with no
  // fallback (ADR-0015), so without this the self-hosted path would fail on the
  // very next command with a key the reviewer had no way to know they needed.
  // Preserved across re-runs — regenerating it would silently invalidate every
  // live session.
  ...
  try {
    ...
    encryptionKey = existing.match(/^SESSION_ENCRYPTION_KEY=(.+)$/m)?.[1]?.trim() ?? '';
    if (encryptionKey) console.log('[bootstrap] reusing existing SESSION_ENCRYPTION_KEY');
  } catch {
    // no previous .env.local
  }
  if (!encryptionKey) {
    encryptionKey = crypto.randomBytes(32).toString('base64');
    console.log('[bootstrap] generated a new SESSION_ENCRYPTION_KEY');
  }

  await fs.writeFile(
    envPath,
    [
      '# Generated by `npm run bootstrap` (self-hosted Appwrite path).',
      '# Gitignored — contains a real API key and encryption key.',
      '# These OVERRIDE .env, so delete this file if you switch to Appwrite Cloud.',
