...
// Fixed timestamp so generated bytes are reproducible. Without it pdf-lib stamps
// a creation date and jszip stamps ZIP entry dates, so re-seeding — or seeding
// the two backends separately — yields files that differ byte-for-byte while
// being otherwise identical. Deterministic output lets the custom and Appwrite
// backends be shown to serve exactly the same bytes. [ADR-0013]
const FIXED_DATE = new Date('2025-01-01T00:00:00Z');

// createFolders:false suppresses implicit directory entries, which jszip would
// otherwise stamp with the current time — the last source of nondeterminism.
const ZIP_ENTRY_OPTS = { date: FIXED_DATE, createFolders: false };
